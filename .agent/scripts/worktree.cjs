#!/usr/bin/env node
/**
 * Git Worktree Manager for Antigravity-HTKit
 ...
 *   --worktree-root <path> Explicit worktree directory (the agent's decision)
...
// --worktree-root: explicit override for worktree location (the agent's decision)
...
function getWorktreeRoot(gitRoot, isMonorepo, explicitRoot = null) {
  // Priority 0: Explicit --worktree-root flag (the agent's decision)
  if (explicitRoot) {
...
function cmdCreate() {
  ...
  // explicitWorktreeRoot comes from --worktree-root flag (the agent's decision)
