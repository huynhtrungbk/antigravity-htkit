#!/usr/bin/env python3
"""
Antigravity-HTKit Help Command - All-in-one guide with dynamic command discovery.
Scans .agent/commands/ directory to build catalog at runtime.

Usage:
    python ht-help.py                    # Overview with quick start
    python ht-help.py fix                # Category guide with workflow
    python ht-help.py plan:fast          # Command details
    python ht-help.py debug login error  # Task recommendations
    python ht-help.py auth               # Search (unknown word)
"""
...
TASK_MAPPINGS = {
    ...
    "config": ["config", "configure", "settings", "ht.json", ".ht.json", "setup", "locale", "language", "paths"],
...
CATEGORY_GUIDES = {
    ...
    "coding-level": {
        ...
        "tip": "Set in .ht.json. Guidelines auto-inject on session start",
    },
    "config": {
        "title": "Antigravity-HTKit Configuration (.ht.json)",
        "workflow": [
            ("Global", "Set user prefs in `~/.agent/.ht.json`"),
            ("Local", "Override per-project in `./.agent/.ht.json`"),
...
def discover_commands(commands_dir: Path, prefix: str) -> dict:
    """Scan .agent/commands/ and build command catalog."""
    commands = {}
    categories = {}
...
def show_overview(data: dict, prefix: str) -> None:
    ...
    help_cmd = f"/{prefix}ht-help" if prefix else "/ht-help"
...
def show_config_guide() -> None:
    """Display comprehensive .ht.json configuration guide."""
    emit_output_type("comprehensive-docs")

    print("# Antigravity-HTKit Configuration (.ht.json)")
    print()
    print("**Locations (cascading resolution):**")
    print("- Global: `~/.agent/.ht.json` (user preferences)")
    print("- Local: `./.agent/.ht.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.agent/.ht.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.agent/.ht.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print()
    print("When both are set, the agent thinks in one language but responds in another.")
    ...
    print("```bash")
    print("# ~/.agent/.ht.json - applies everywhere")
    print("cd /tmp/new-project && agent  # Uses global config")
    print("```")
    print()
    print("**Project with local override:**")
    print("```bash")
    print("# Global: issuePrefix = \"GH-\"")
    print("# Local (.agent/.ht.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("# Coding Level (Adaptive Communication)")
    print()
    print("Adjusts the agent's communication style based on user's experience level.")
    ...
    print("- Global: `~/.agent/.ht.json` - personal preference")
    print("- Local: `./.agent/.ht.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    print()
    print("1. SessionStart hook reads `codingLevel` from `.ht.json`")
...
def main():
    # Find .agent/commands directory
    script_path = Path(__file__).resolve()
    agent_dir = script_path.parent.parent  # .agent/scripts -> .agent
    commands_dir = agent_dir / "commands"

    if not commands_dir.exists():
        print("Error: .agent/commands/ directory not found.")
        sys.exit(1)
    ...
    if not data["commands"]:
        print("No commands found in .agent/commands/")
        sys.exit(1)
    ...
    if input_str.lower() in ["config", "configuration", ".ht.json", "ht.json"]:
        show_config_guide()
        return
