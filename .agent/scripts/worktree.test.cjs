 ...
 * Run: node .agent/scripts/worktree.test.cjs
 */
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'antigravityhtkit-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from antigravityhtkit-engineer submodule
  const submodulePath = '/home/huynhtrungbk/antigravityhtkit/antigravityhtkit-engineer';
...
test('create with partial project match in monorepo', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'cli' should match 'antigravityhtkit-cli'
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'antigravityhtkit-cli', 'Should find antigravityhtkit-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'antigravityhtkit' matches multiple projects
  const result = run('create antigravityhtkit test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
