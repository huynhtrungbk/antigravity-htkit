 ...
 * 1. Project-scope: .agent/secrets/google_client_secret.json
 * 2. Global (Unix): ~/.agent/secrets/google_client_secret.json
 * 3. Global (Windows): %USERPROFILE%\.agent\secrets\google_client_secret.json
 *
 * Also supports service account credentials:
 * 1. Project-scope: .agent/secrets/ga_service_account.json
 * 2. Global: ~/.agent/secrets/ga_service_account.json
 *
...
/**
 * Get project root (searches upward for .agent directory)
 */
...
function getClientSecretPaths() {
  ...
  return [
    ...
    path.join(projectRoot, '.agent', SECRETS_DIR, CLIENT_SECRET_FILENAME),
    // 2. Global user scope
    path.join(homeDir, '.agent', SECRETS_DIR, CLIENT_SECRET_FILENAME),
  ];
}
...
function getServiceAccountPaths() {
  ...
  return [
    // 1. Project-scope (highest priority)
    path.join(projectRoot, '.agent', SECRETS_DIR, SERVICE_ACCOUNT_FILENAME),
    // 2. Global user scope
    path.join(homeDir, '.agent', SECRETS_DIR, SERVICE_ACCOUNT_FILENAME),
  ];
}
...
function getTokenPaths() {
  ...
  return [
    // 1. Project-scope (highest priority)
    path.join(projectRoot, '.agent', SECRETS_DIR, GA_TOKEN_FILENAME),
    // 2. Global user scope
    path.join(homeDir, '.agent', SECRETS_DIR, GA_TOKEN_FILENAME),
  ];
}
...
Credential files (priority order):
  1. PROJECT/.agent/secrets/ga_service_account.json (service account)
  2. ~/.agent/secrets/ga_service_account.json
  3. PROJECT/.agent/secrets/google_client_secret.json (OAuth)
  4. ~/.agent/secrets/google_client_secret.json

Token file:
  1. PROJECT/.agent/secrets/ga_token.json
  2. ~/.agent/secrets/ga_token.json
`);
  } else {
