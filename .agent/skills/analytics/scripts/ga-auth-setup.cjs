 ...
 *
 * Options:
 *   --scope=global    Save token globally (~/.agent/secrets/)
 *   --scope=project   Save token to project (.agent/secrets/)
 *   --status          Check current auth status
 *   --help            Show help
 *
 * Requires: googleapis
 * Credentials: google_client_secret.json in .agent/secrets/
 */
...
function showStatus() {
  ...
  if (status.serviceAccountFound) {
    ...
    console.log('1. Create a service account and save to .agent/secrets/ga_service_account.json');
    console.log('2. Download OAuth client secret to .agent/secrets/google_client_secret.json');
  }
}
...
Options:
  --scope=global    Save token to ~/.agent/secrets/
  --scope=project   Save token to .agent/secrets/ (default)
  --status          Check current auth status
  --help            Show this help
...
Prerequisites:
  ...
  3. Place in .agent/secrets/google_client_secret.json
