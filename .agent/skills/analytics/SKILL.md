...
Credentials: `.agent/secrets/ga_service_account.json` or `google_client_secret.json`

### Scripts
| Script | Purpose |
|--------|---------|
| `scripts/ga-config-loader.cjs` | Load credentials from .agent/secrets |
...
```bash
# List accounts
node .agent/skills/analytics/scripts/ga-list-accounts.cjs --summaries

# Run report
node .agent/skills/analytics/scripts/ga-run-report.cjs \
