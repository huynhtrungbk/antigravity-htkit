...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.agent/.ht.json` (or `~/.agent/.ht.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `.agent/.ht.json`: `gemini.model` (default: `gemini-3-flash-preview`)
