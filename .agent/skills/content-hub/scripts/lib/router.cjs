...
const { enhanceContent, generateContent, isthe agentAvailable } = require('./ai-bridge.cjs');
  ...
  return async function route(req, res) {
    ...
    try {
      ...
      if (pathname === '/api/ai/status') {
        const available = await isthe agentAvailable();
        sendJson(res, { available, message: available ? 'Agent Kit ready' : 'Agent Kit not found' });
        return;
      }
