...
2. Download JSON → save as `.agent/secrets/google_client_secret.json` (project) or `~/.agent/secrets/` (global)
3. Authenticate:
```bash
...
Config: `google_client_secret.json` in `.agent/secrets/` (project or `~/.agent/secrets/` global)
