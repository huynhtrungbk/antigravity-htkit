#!/usr/bin/env node
/**
 * Environment Variable Loader for Agent Kit Skills
 *
 * Loads environment variables following the priority order:
 * 1. process.env (highest priority - already set)
 * 2. $HOME/.agent/skills/${SKILL}/.env (skill-specific)
 * 3. $HOME/.agent/skills/.env (shared across skills)
 * 4. $HOME/.agent/.env (global the agent config)
 *
 * Usage:
 *   const { loadEnv, getEnv } = require('./env-loader.cjs');
 *   loadEnv('seo-optimization'); // Load with skill name
...
function loadEnv(skillName) {
  ...
  const envPaths = [
    path.join(homeDir, '.agent', '.env'), // 4. Global the agent config (lowest)
    path.join(homeDir, '.agent', 'skills', '.env'), // 3. Shared skills
  ];

  // Add skill-specific .env if skill name provided
  if (skillName) {
    envPaths.push(
      path.join(homeDir, '.agent', 'skills', skillName, '.env') // 2. Skill-specific
    );
  }
