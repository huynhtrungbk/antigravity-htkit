...
function getConfigPaths() {
  ...
  return [
    ...
    path.join(projectRoot, '.agent', SECRETS_DIR, CONFIG_FILENAME),
    // 2. Global user scope
    path.join(homeDir, '.agent', SECRETS_DIR, CONFIG_FILENAME),
  ];
}
...
function getTokenPaths() {
  ...
  return [
    // 1. Project-scope (highest priority)
    path.join(projectRoot, '.agent', SECRETS_DIR, TOKEN_FILENAME),
    // 2. Global user scope
    path.join(homeDir, '.agent', SECRETS_DIR, TOKEN_FILENAME),
  ];
}
...
Searched paths (priority order):
  1. PROJECT/.agent/secrets/google_client_secret.json
  2. ~/.agent/secrets/google_client_secret.json (or %USERPROFILE% on Windows)
`);
  } else {
