...
## Quick Start (Antigravity-HTKit Scripts)

Antigravity-HTKit provides cross-platform scripts for GSC integration. Use these for most tasks:
...
**Config location:** `.agent/secrets/google_client_secret.json` (project) or `~/.agent/secrets/` (global)
...
8. Place in .agent/secrets/ (project scope) or ~/.agent/secrets/ (global)
```
...
**Step 3: Authenticate with Antigravity-HTKit Script**
```bash
