...
node screenshot.js --url https://example.com --full-page true \
  --output .agent/chrome-devtools/screenshots/seo-audit.png
```
...
node screenshot.js --url https://example.com \
  --output .agent/chrome-devtools/screenshots/atf.png
```
...
```bash
node evaluate.js --script "page.setViewport({width: 375, height: 812, isMobile: true})"
node screenshot.js --output .agent/chrome-devtools/screenshots/mobile.png
```
...
OUT=".agent/chrome-devtools/seo-audits/$SESSION"
mkdir -p "$OUT"
