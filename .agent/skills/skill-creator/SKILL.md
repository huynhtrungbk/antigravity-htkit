---
name: skill-creator
description: Create or update the agent skills. Use for new skills, skill references, skill scripts, optimizing existing skills, extending the agent's capabilities.
...
**IMPORTANT:**
- Skills are not documentation, they are practical instructions for Agent Kit to use the tools, packages, plugins or APIs to achieve the tasks.
- Each skill teaches the agent how to perform a specific development task, not what a tool does.
- Agent Kit can activate multiple skills automatically to achieve the user's request.
...
- Descriptions in metadata of `SKILL.md` files should be both concise (**less than 200 characters**) and still contains enough usecases of the references and scripts, this will help skills can be activated automatically during the implementation process of Agent Kit.
...
**Why?**
Better **context engineering**: leverage **progressive disclosure** technique of Agent Skills, when agent skills are activated, Agent Kit will consider to load only relevant files into the context, instead of reading all long `SKILL.md` as before.
...
- **Benefits**: Keeps SKILL.md lean, loaded only when the agent determines it's needed, makes information discoverable without hogging the context window.
...
**IMPORTANT:**
- Referenced markdown files should be also **less than 150 lines**, remember that you can always split them into multiple files (**progressive disclosure** principle).
- Referenced markdown files are practical instructions for Agent Kit to use the tools, packages, plugins or APIs to achieve the tasks.
- Each skill teaches the agent how to perform a specific development task, not what a tool does.
