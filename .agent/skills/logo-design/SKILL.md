...
```bash
python3 ~/.agent/skills/logo-design/scripts/search.py "tech startup modern" --design-brief -p "BrandName"
```
...
```bash
# Search styles
python3 ~/.agent/skills/logo-design/scripts/search.py "minimalist clean modern" --domain style

# Search color palettes
python3 ~/.agent/skills/logo-design/scripts/search.py "tech professional trust" --domain color

# Search industry guidelines
python3 ~/.agent/skills/logo-design/scripts/search.py "healthcare medical" --domain industry
```
...
If user selects **Yes**, invoke /ui-ux skill to create an HTML file in the output directory with:
