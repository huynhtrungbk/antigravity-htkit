...
3. `ui-ux` (if `frontend-design` is not available) - Premium UI patterns, component library
...
1. Embed design tokens inline (for `file://` compatibility)
   ```bash
   node .agent/skills/design-system/scripts/embed-tokens.cjs --minimal
   ```
...
```bash
python .agent/skills/design-system/scripts/search-slides.py "[goal]" -d strategy
```

### 3. Contextual Search Per Slide
```bash
python .agent/skills/design-system/scripts/search-slides.py "[slide goal]" \
...
**Data location:** `.agent/skills/design-system/data/`
...
```bash
/slides:create "10-slide seed funding pitch for Antigravity-HTKit"
/slides:create "B2B sales deck with ROI focus"
/slides:create "5-slide product demo with features"
```

**Skill Dependencies:** frontend-design, ui-ux, design-system, brand-guidelines
